...
The Server Key is used only for server-side token validation and is not intentionally exposed to website visitors.
...
* PHP 7.4 or later
* A Yandex Cloud account with SmartCaptcha configured
* Valid SmartCaptcha Client Key and Server Key
...
The plugin follows WordPress security practices for validation, sanitization, escaping and permission checks.
